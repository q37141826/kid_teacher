 ...
 * 影音文件列表
 * (.avi|.mp4|.mpeg|.mpg|.mov|.flv|.swf|.rmvb
 * *(.mp3|.wma)
 */
public class FrMedia extends FxFragment {
    ...
    protected View initinitLayout(LayoutInflater inflater) {
        return inflater.inflate(R.layout.fr_media, null);
    }
    ...
    public void onViewCreated(View view, Bundle savedInstanceState) {
        ...
        String data = new FileUtil().getData(UserController.getInstance().getNoticeVideoLocalList(getContext()));
        ...
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (onFragment!=null)onFragment.onToMessage(1,choiseFiles);
            }
        });
    }
        ...
        if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
            new AsyncTask<Void, Void, Void>() {
                ...
                protected void onPostExecute(Void result) {
                    ...
                    new FileUtil().saveData(UserController.getInstance().getNoticeVideoLocalList(getContext()), new GsonUtil().getJsonElement(allFiles).toString());
                    finishRefreshAndLoadMoer(refreshLayout, 1);
                    if (adapter != null)
                        adapter.notifyDataSetChanged();
                }
    ...
    private void readerAllFiles(File dir) {
        ...
        for (File f : fs) {
            ...
            if ((f.isFile()&&f.length()>0)&&(StringUtil.isFileType(path, StringUtil.videoType) || StringUtil.isFileType(path, StringUtil.audioType))) {
                allFilesTemp.add(f);
            }
